...
/**
 * Sincroniza todos os pagamentos de um link e busca no Mercado Pago por
 * external_reference quando o registro local não encontra confirmação.
 *
 * Falhas são registradas no console — sem isso um erro do gateway faz o
 * checkout parecer que "ignora" o pagamento.
 */
export async function syncPaymentsForLink(linkId: string): Promise<{
  ...
  function record(context: string, err: unknown) {
    const message = (err as Error).message;
    errors.push(message);
    console.error(`[flowdesk] sync link ${linkId} — ${context}: ${message}`);
  }

  // 1. Consulta direta dos pagamentos que o FlowDesk já conhece.
  const { data: localPayments } = await db()
    .from("payments")
  ...
  const unresolved = (localPayments ?? []).filter(
    (row) => !["APPROVED", "AUTHORIZED", "REFUNDED", "CHARGED_BACK"].includes(String(row.status))
  );
  ...
  for (const payment of unresolved) {
    try {
      ...
      record(`consulta do pagamento ${payment.provider_payment_id}`, err);
    }
  }

  // 2. Rede de segurança: o pagador pode ter gerado outro pagamento no MP
  //    (ex.: QR reaberto em outro dispositivo) que o banco local não conhece.
  if (!approved) {
    try {
      ...
      for (const remote of remotePayments) {
        ...
        try {
          await syncMercadoPagoPayment(String(remote.id), { linkId });
          synced++;
          approved = true;
          break;
        } catch (err) {
          record(`gravação do pagamento remoto ${remote.id}`, err);
        }
      }
    } catch (err) {
      record("busca por external_reference", err);
    }
  }
