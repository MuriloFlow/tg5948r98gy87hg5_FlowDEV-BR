...
/**
 * Inclui cobrança + link de pagamento quando há fatura bloqueante em aberto.
 * Bloqueio manual ainda pode (e deve) oferecer pagamento se existir dívida.
 * SUSPENDED/ARCHIVED não exibem link de pagamento.
 */
...
export async function buildEntitlementResponse(projectId: string) {
  ...
  const blockType =
    entitlement.status === "SUSPENDED"
      ...
      : blockedBy
        ? "manual"
        : entitlement.status === "BLOCKED_PAYMENT"
          ? "payment"
          : null;
